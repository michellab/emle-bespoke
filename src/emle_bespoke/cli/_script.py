...
def main():
    ...
    parser.add_argument(
        "--pressure", type=float, default=1.0, help="The simulation pressure in bar."
    )
    parser.add_argument(
        "--timestep",
        type=float,
        default=1.0,
        help="The simulation timestep in femtoseconds.",
    )
    parser.add_argument(
        "--friction_coefficient",
        type=float,
        default=1.0,
        help="The Langevin friction coefficient in picoseconds^-1.",
    )
    ...
    topology_off = create_off_topology(
        args.n_solute, args.n_solvent, args.solvent, args.solute
    )

    print(
        f"Topology: {topology_off.n_molecules}, {topology_off.box_vectors}, {topology_off.get_positions().shape}"
    )
    ...
    print(
        f"Interchange: {interchange.topology.n_atoms}, {interchange.box}, {interchange.positions.shape}"
    )
